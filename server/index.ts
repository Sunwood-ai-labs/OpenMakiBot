...
import { isEffortLevel, type WireBot, type WireGroup } from "../shared/wire.ts";
...
const wireBot = (bot: BotRecord): WireBot => {
  ...
  const visible = approvalGrant && !approvalGrant.threadOnly
    ...
    : rest;
  return { ...visible, waitingForTeammates: activeCoordinationForThread(bot.threadId) && !threadBusy(bot.id, bot.threadId), ...(activeCoordinationForThread(bot.threadId) && !visible.busy ? { busy: true, activity: "working" as const, waitingOnTeammate: true as const } : {}),
