...
const roomHandoffs = new RoomHandoffs(join(DATA_DIR, "room-handoffs.json"), {
  ...
  busy: n => n.participants.some(id => store.bot(id)?.busy) || (!n.groupId
  ...
  run: async (node, resumed, signal) => {
    ...
    const run = (groupQueues.get(group.id) ?? Promise.resolve()).catch(() => {}).then(async () => {
      ...
      for (const speakerId of speakers) {
        ...
        const roleInstructions = node.kind === "discussion"
          ...
          : (node.kind === "assignment" && group.requireRoomDiscussion
        ...
        await runGroupMemberTurn(group.id, node.threadId, speaker.id, MAX_COMMS_DEPTH, new Set(),
          ...
          { claimed: true }, { roomHandoffId: node.id, resumed, systemInstructions, turnInstructions: `${turnText}\n${roleInstructions}`, followMentions: false, result }, operation);
  ...
  try {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/discuss-room") {
        ...
        if (peerReviewRequired(internalSender, internalCapability.threadId)) {
