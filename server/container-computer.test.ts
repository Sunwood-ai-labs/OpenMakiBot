...
describe("managed GUI image namespace", () => {
  it("preserves the default and accepts a separate guide repository", () => {
    expect(managedImageRepository(undefined)).toBe("localhost/openmausbot/cua-local-vm");
    expect(managedImageRepository("localhost/openmausbot-nekoneko/cua-local-vm")).toBe("localhost/openmausbot-nekoneko/cua-local-vm");
    expect(managedImageRepository("registry.example:5000/demo/gui")).toBe("registry.example:5000/demo/gui");
  });
  it.each(["repo/gui:latest", "repo/gui@sha256:abcd", "--flag", "repo/gui other", "repo/../gui"])("rejects invalid image repository %s", value => {
    expect(() => managedImageRepository(value)).toThrow("OMB_VM_IMAGE_REPOSITORY");
  });
  it("embeds a checksum-pinned Japanese font and its license before startup", () => {
