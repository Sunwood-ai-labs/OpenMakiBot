...
if (!podman) throw Error('Set OMB_VERIFY_PODMAN to the Podman executable');
const connection=JSON.parse(execFileSync(podman,['system','connection','list','--format','json'],{encoding:'utf8'})).find(x=>x.Name===(process.env.OMB_VERIFY_MACHINE || 'openmausbot'));
...
const api=async(path,body,method=body?'POST':'GET')=>{const r=await fetch(fixture.info.url+path,{method,headers:{'content-type':'application/json'},...(body?{body:JSON.stringify(body)}:{})});if(!r.ok)throw Error(path+': '+await r.text());return r.json()};
const ids=[];const evidence=[];
try{
 ...
 for(const name of ['Fixture GUI A','Fixture GUI B']){
  const b=(await api('/api/bots',{name})).bot;ids.push(b.id);
  await api('/api/bots/'+b.id,{computer:'vm'},'PATCH');
  await api('/api/bots/'+b.id+'/local-computer/run',{});
  const vm=await api('/api/bots/'+b.id+'/local-computer');
 ...
 const off=ids[0];await api('/api/bots/'+off,{computer:'off'},'PATCH');
 ...
 mkdirSync('docs/verification/evidence',{recursive:true});writeFileSync('docs/verification/evidence/group-vm-routing.json',JSON.stringify({passed:true,fixture:fixture.info.url,evidence,computerOff:true},null,2));
 console.log(JSON.stringify({passed:true,evidence,computerOff:true}));
}finally{
 for(const id of ids)await api('/api/bots/'+id+'/local-computer/remove',{}).catch(e=>console.error(e.message));
 await fixture.close();
}
