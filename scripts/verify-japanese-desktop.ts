...
async function capture() {
  ...
  for (const attempt of [1, 2]) {
    ...
    // Standalone font patches need Firefox's capability prerequisite; the
    // combined reproduction branch already supplies it in production args.
    if (!args.includes("SYS_CHROOT")) args.splice(args.length - 1, 0, "--cap-add", "SYS_CHROOT");
