...
export async function verifyRoomDiscussion(output: string, preview = false, live = false) {
  ...
  try {
    const bots: any[] = [];
    for (const [name, title, section] of [
      ["ミナト", "経営会議の議長。議論して方針と対象範囲を決める", "経営"],
      ["アオイ", "営業責任者。顧客価値と優先順位を検討", "経営"],
      ["ユイ", "管理責任者。予算と情報管理のリスクを指摘", "経営"],
      ["レン", "開発部長。部内議論で要件を設計と受入条件に具体化", "開発"],
      ["リツ", "設計担当。方式の比較と実装の複雑さを評価", "開発"],
      ["マコ", "品質責任者。漏れや失敗条件を指摘", "開発"],
      ["ソラ", "実装チームリーダー。チーム議論で実装・テスト計画をまとめる", "実装"],
      ["ヒナ", "実装担当。具体的な処理方式と代案を提案", "実装"],
      ["ナギ", "テスト担当。他の意見を検討し境界値と確認手順を提案", "実装"],
    ]) {
      const bot = (await control("new-bot", "--name", name, "--title", title, "--section", section)).bot;
