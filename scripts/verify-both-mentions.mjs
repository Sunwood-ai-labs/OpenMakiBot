...
const out = new URL(`../docs/verification/evidence/both-mentions-20260913/${glm ? 'glm/' : live ? 'live/' : ''}`, import.meta.url);
...
try {
  ...
  await tool('update_channel', {channel_id:room.id, default_responder:{kind:'mentions'}});
  const requests=[
    {transport:'HTTP',text:'@API検証Bot 137+286はいくつですか？日本語で短く答えてください。'},
    {transport:'MCP stdio',text:'@MCP検証Bot API検証Botの答えを2倍するといくつ？日本語で短く答えてください。'}
  ];
  ...
  assert.equal(replies[0].from?.botId,a.id); assert.equal(replies[1].from?.botId,b.id);
  assert.ok(replies[0].text.includes('423'));
  assert.ok(replies[1].text.includes('846'));
  assert.equal(group.channel.defaultResponder.kind,'mentions');
