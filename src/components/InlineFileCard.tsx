...
export function InlineFileCard({ kind, label, caption, file, error, visible, expanded, onExpand }: {
  kind: FilePreviewKind; label: string; caption?: ReactNode;
  ...
  const badge = document ? kind === 'presentation' ? 'PPTX' : kind === 'pdf' ? 'PDF' : 'TABLE' : null;
  ...
  return <span className={`group/file my-1 inline-flex ${document ? 'w-48' : 'w-64'} max-w-full flex-col overflow-hidden rounded-xl border border-hairline/50 bg-inset text-left align-top`}>
    <span className={`relative block aspect-video w-full overflow-hidden ${document ? `bg-inset p-2 ${kind === 'spreadsheet' ? 'pt-6' : ''}` : 'bg-black/90'}`}>
      ...
      {badge && <span aria-hidden="true" className="pointer-events-none absolute left-3 top-3 rounded border border-hairline/50 bg-card/95 px-1.5 py-0.5 text-[9px] font-semibold tracking-wide text-ink-secondary">{badge}</span>}
    </span>
