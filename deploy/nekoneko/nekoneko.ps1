...
function Read-GuideSettings {
  $values = @{ OMB_PODMAN_MACHINE='openmausbot'; COMPOSE_PROJECT_NAME='nekoneko-guide'; OMB_PORT='29799'; OMB_HTTP_PORT='29880'; OMB_GLM_MODEL='glm-5.3' }
...
try {
  ...
  switch ($Action) {
    'up' { Invoke-Compose up -d --build; Wait-GuideReady }
    'configure' {
      Wait-GuideReady
    ...
    'stop' {
      if (Get-RunningApp) { Invoke-Compose exec -T omb node /opt/nekoneko/team.mjs stop-desktops }
      Invoke-Compose stop
    }
    'doctor' {
